...
function transformData(signalsData) {
  ...
  signalsData.cells.forEach((cell) => {
    if (cell.type === "fpga_interconnect") {
      ...
      if (
        cell.start.toLowerCase().includes("clk") ||
        cell.start.toLowerCase().includes("clock")
      ) {
      ...
      const node = {
        id: cell.start,
        type: cell.type.toLowerCase().includes("dff")
          ? "DFF"
          : cell.type.toLowerCase().includes("lut")
          ? "LUT"
          : cell.type.toLowerCase().includes("clk")
          ? "CLOCK"
          : cell.type,
      };
